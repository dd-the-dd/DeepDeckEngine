...
struct OracleDocument<'a> {
    face_id: Option<&'a str>,
    face_name: &'a str,
    face_type_line: &'a str,
    oracle_text: &'a str,
}

fn documents(request: &OracleCardParseRequest) -> Vec<OracleDocument<'_>> {
...
fn normalize_oracle_encoding(text: &str) -> String {
    text.replace("â€”", "—")
        .replace("â€¢", "•")
        .replace("âˆ’", "−")
...
fn split_abilities(document: &OracleDocument<'_>) -> Vec<AbilitySource> {
